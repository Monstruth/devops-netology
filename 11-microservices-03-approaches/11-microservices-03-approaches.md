...
Сам пользовался только Jenkins и Gitlab. Могу сказать что Gitlab намного удобнее, потому что это полноценная система, это мощный комбайн, там есть всё для DevOps от начала(разработка и хранение кода) до конца(выкатка готовой версии на "боевой" сервер). 
